...
export default class Slot {
  ...
  public async spin(): Promise<boolean> {
    ...
    setTimeout(() => {
      const winnerLeft = document.getElementById(
        "winner-info-left",
      ) as HTMLElement;
      const winnerRigth = document.getElementById(
        "winner-info-right",
      ) as HTMLElement;
      const newWinner = document.createElement("div");
      newWinner.style.cursor = "pointer";
      newWinner.onclick = function (event) {
        const target = event.currentTarget as HTMLElement; // Cast to HTMLElement
        target.remove(); // Now this refers to the clicked element
        console.log("Element removed");
      };
      newWinner.innerHTML = randomNames[randomNames.length - 1];
      if (this.count % 2 === 0) {
        winnerRigth.appendChild(newWinner);
      } else {
        winnerLeft.appendChild(newWinner);
      }
    }, 3500);
